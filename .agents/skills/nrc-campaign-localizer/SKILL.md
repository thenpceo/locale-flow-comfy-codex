...
3. For two or more locales, submit independent per-locale graphs as one batch. Keep one prompt ID, retry boundary, output folder, and QA state per locale.
4. Wait for terminal status. Never infer completion from queue counts and never rerun a paid job because a download failed.
5. Download original city plate, raw runner, runner alpha, validation composite, runner-only motion MP4, and strategist JSON. Record checksums in each handoff.
6. Invoke `nrc-static-finisher` for every completed static handoff.
7. Invoke `nrc-motion-finisher` after the static design is approved, so the motion design inherits the exact same type geometry.
8. Run mechanical QA. Keep native-language, cultural, casting, landmark, brand, and rights decisions marked for human review.
